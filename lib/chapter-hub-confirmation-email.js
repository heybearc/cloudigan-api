/**
 * Customer confirmation email for BNI Chapter Hub subscription purchases.
 */
...
function buildChapterHubConfirmationEmail(data) {
  ...
  const productName = data.productName || BNI_CHAPTER_HUB_NAME;
  ...
  const subject = `Welcome to ${BNI_CHAPTER_HUB_NAME} — ${productName}`;
        ...
        <table cellpadding="0" cellspacing="0" border="0" style="width: 600px; max-width: 600px; border-radius: 8px; overflow: hidden;">
          <tr>
            <td style="padding: 40px 30px; text-align: center; border-bottom: 3px solid #3b82f6;">
              ...
              <p style="margin: 0 0 8px 0; padding: 0; color: #1e3a8a; font-size: 22px; font-weight: 700;">${escapeHtml(BNI_CHAPTER_HUB_NAME)}</p>
              <p style="margin: 0; padding: 0; color: #475569; font-size: 14px;">Powered by Cloudigan IT Solutions</p>
            </td>
          </tr>
          <tr>
            <td style="padding: 35px;">
              <p style="margin: 0 0 20px 0; font-size: 19px; color: #1e3a8a; font-weight: 600;">Hi ${escapeHtml(firstName)},</p>
              <p style="margin: 0 0 15px 0; font-size: 15px; line-height: 1.6; color: #2c3e50;">Thank you for subscribing to <strong>${escapeHtml(BNI_CHAPTER_HUB_NAME)}</strong>. This email confirms your plan.</p>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="margin: 20px 0;">
                <tr>
                  <td align="center">
                    <a href="${escapeHtml(appUrl)}" style="display: inline-block; padding: 14px 32px; background-color: #3b82f6; color: #ffffff; text-decoration: none; border-radius: 6px; font-weight: 700; font-size: 15px;">Open ${escapeHtml(BNI_CHAPTER_HUB_NAME)}</a>
                  </td>
                </tr>
              </table>
              <p style="margin: 0 0 20px 0; font-size: 15px; line-height: 1.6; color: #2c3e50;">Sign in anytime at <a href="${escapeHtml(appUrl)}" style="color: #3b82f6; text-decoration: none;">hub.cloudigan.net</a>. For help with your account, email <a href="mailto:${escapeHtml(supportEmail)}" style="color: #3b82f6; text-decoration: none;">${escapeHtml(supportEmail)}</a>. ${escapeHtml(NO_REPLY_NOTE)}</p>
              <p style="margin: 30px 0 0 0; font-size: 15px; color: #2c3e50;">Thanks for choosing ${escapeHtml(BNI_CHAPTER_HUB_NAME)}.</p>
          ...
          <tr>
            <td style="background-color: #1e293b; padding: 30px; text-align: center;">
              ...
              <p style="margin: 0; font-size: 13px;"><a href="${escapeHtml(appUrl)}" style="color: #60a5fa; text-decoration: none;">hub.cloudigan.net</a> · <a href="https://www.cloudigan.com" style="color: #60a5fa; text-decoration: none;">www.cloudigan.com</a></p>
            </td>
          </tr>
        </table>
      </td>
...
Thank you for subscribing to ${BNI_CHAPTER_HUB_NAME}. This email confirms your plan.
...
Open ${BNI_CHAPTER_HUB_NAME}: ${appUrl}

Sign in anytime at hub.cloudigan.net

For help with your account, email ${supportEmail}. ${NO_REPLY_NOTE}

Thanks for choosing ${BNI_CHAPTER_HUB_NAME}.
– The Cloudigan Team
