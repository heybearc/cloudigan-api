...
function classifyProduct(productName = '', productId = null) {
  ...
  const isService = SERVICE_KEYWORDS.some((keyword) => lower.includes(keyword));
  return isService ? 'service' : 'rmm';
}
...
function buildAdminActionSummary(profileId, results) {
  ...
  if (profileId === 'rmm') {
    ...
    if (results.wixCmsSkipped && results.wixConfigured === false) {
      // omit — not configured, don't claim
    }
  } else {
